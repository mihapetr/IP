...
* popravljene greške prilikom imena `nat` i `int` varijabli, stavljen uvjet na postojanje iteratorske varijable unutar `for` petlje samo u dosegu bloka, implementirana `foreach` petlja koja može prolaziti svjetovima i prop. varijablama definiranog modela, implementiran operator ostatka `%`, onemogućeno potenciranje na negativan eskponent (u jeziku se radi samo s cijelim i prirodnim brojevima), funkcija *unos_programa(datoteka.txt)* za unos već napisanog programa u .txt datoteci, naredba `spremi <ime-modela>` koja ispisuje valuaciju i relaciju deklariranog modela, dodana *ispis_varijabla* `nr` za prijelaz u novi red (14.6.2023.)
